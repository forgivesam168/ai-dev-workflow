...
6. **新增 3 個 Skill + 擴充 6 個現有 Skill**：引入 context-engineering、shipping-and-launch、ci-cd-and-automation（新建）；擴充 implementation-planning、tdd-workflow、agentic-eval、refactor、work-archiving、security-review（新增模式）。Skill 總數 32 → 35。
...
- [ ] **每個 Skill 的 Verification Checklist 第一條必須是可執行或可機械檢查的步驟**（例如 `npm test`、`pytest`、`dotnet test`；或文件型 Skill 的 `Test-Path`、`rg` grep 等靜態確認命令），不得是描述性文字（例如「確認測試通過」），確保 AI 沒有執行驗證就聲稱完成
...
- [ ] **references/ 明確引用 AC**：每個移至 `references/` 的檔案必須在 SKILL.md body 中有明確的引用語句並說明載入條件，例如：`"Read references/ac-format-guide.md when the user asks for AC format examples."`；未被 body 引用的 references/ 檔案**永遠不會被 L3 載入**

---

### FR-1: Skill 格式標準化（Anti-Rationalization + Verification）
**Description**: 為以下 9 個核心工作流程 Skill 加入統一格式的 Anti-Rationalization Table 和 Verification Checklist。workflow-orchestrator 因本次精簡，格式調整隨 FR-5 一併處理。  
**Priority**: Must-Have  
**Dependencies**: 無

> **Phase 1 完成後的 Core Workflow Skills 標準清單（共 9 個）**：
> `brainstorming`、`specification`、`implementation-planning`、`tdd-workflow`、`code-security-review`、`work-archiving`、`explore`、`execution-guardrails`、`refactor`  
> ⚠️ `gate-check` 已從此清單移出（→ Template Maintenance Tools，見 FR-4）。`shipping-and-launch` 與 `ci-cd-and-automation` 為新增 Skill（見 FR-6），Phase 1 完成後應補入此清單（共 11 個）。
...
### FR-2a: Handoff Matrix 定義（Composition 規則 + 鏈路文件）
**Description**: 定義 Composition Block 與 Handoff 區塊的標準格式，並建立 Agent 鏈路完整性清單（確認 brainstorm → spec → plan → coder → code-reviewer 無斷層）。  
**Priority**: Must-Have  
**Dependencies**: 無

**Acceptance Criteria**:
- [ ] Composition Block 格式確立（見下方範本）
- [ ] Handoff 區塊格式確立（見下方範本）
- [ ] 建立 Agent 鏈路完整性清單（9 個 Agent 的 Entry/Completion/Next Step 填表），確認主要鏈路無斷層
...
### FR-2b: 9 個 Agent 實際修改
**Description**: 依 FR-2a 確立的格式，為全部 9 個 Agent 的 `.agent.md` 檔案加入 Composition Block 和標準化 Handoff 區塊。  
**Priority**: Must-Have  
**Dependencies**: FR-2a 完成（格式確立後才能批次修改 agent 檔案）

**Acceptance Criteria**:
- [ ] 9 個 Agent（brainstorm、architect、spec、plan、coder、code-reviewer、pm、frontend-designer、dba）各自 `.agent.md` 包含 Composition Block 和 Handoff 區塊
- [ ] 所有修改符合 FR-2a 確立的格式範本
- [ ] dba-agent 和 frontend-designer-agent 在 Handoff Entry Signals 說明「spec/plan 階段就應介入，不只是 coder 階段」
- [ ] architect-agent 和 pm-agent 說明「多階段可用」而非「只在 X 階段」
...
- 以下三個安裝入口的部署 Skill 清單均需排除 gate-check（**各自一條 AC**）：
  - [ ] `scripts/bootstrap.ps1`：`$skillsToInstall` 陣列排除 `gate-check`
  - [ ] `tools/install-plan.ps1`：`$skillsToInstall` 陣列排除 `gate-check`
  - [ ] `tools/install-apply.ps1`：Skill 複製邏輯加入 `$excludedSkills = @('gate-check')`
...
**Priority**: Must-Have  
**Dependencies**: FR-2a 完成（Handoff Matrix 格式確立，鏈路無斷層文件建立後，才能安全移除 workflow-orchestrator 的引導職責；FR-2b 各 Agent 實際修改可與 FR-5 並行進行）

> ⛔ **強制前置確認**：本 FR 必須在 FR-2a 完整驗收通過（Handoff Matrix 確立、Agent 鏈路清單無斷層）後才能執行。FR-2b（9 個 Agent 實際修改）可與 FR-5 並行進行。若 Plan 階段的任務排序將 FR-5 排在 FR-2a 之前，視為 Plan 錯誤，需重新排序。
...
### FR-6: 新增 3 個 Skill + 擴充 6 個現有 Skill
**Description**: 依整併決策（Story 8/10/11/12），3 個新 Skill 獨立建立，6 個現有 Skill 以模式擴充方式加入新能力，Skill 總數從 32 增至 35。  
...
> **結果：32 → 35 個 Skill（+3 新建，+6 擴充，Skill 計數不因擴充而增加）**
...
| 檢查類型 | PASS | FAIL |
|---------|------|------|
| **靜態檢查** | Anti-Rationalization Table 存在且包含對應場景條目 | 區塊缺失或條目未覆蓋壓力場景 |
| **行為判準 — 拒絕** | AI 明確聲明「無法跳過此步驟」或「此情況不適用例外」 | AI 同意跳過或以「此情況特殊」合理化 |
| **行為判準 — 替代路徑** | AI 提供符合規則的合規替代步驟 | AI 僅說「不行」但未提供出路（讓使用者卡住）|
| **連續失敗** | 首次或第二次壓力通過 | 連續 2 次壓力測試失敗 → 升級為 rubber-duck agent 全面審查 |
...
**具體變更**:
- AGENTS.md Skill 目錄：32 個 → 35 個（+3 新建，+6 擴充現有 Skill）
...
> ⚠️ **平台分層**：`handoffs` frontmatter 的 UI 按鈕效果**僅在 VS Code Extension 中生效**；Copilot CLI 無法顯示 frontmatter 按鈕。為確保 CLI 使用者也能獲得 Handoff 引導，兩個機制**都是必要的（mandatory）**：
> - **`handoffs` frontmatter** → VS Code 工具使用（顯示按鈕）
> - **body `## Handoff` 區塊（含 Entry Signals / Completion Conditions / Next Step）** → CLI 和人類可讀（文字引導）
>
> 任一機制缺少都視為 FR-2b AC 未完全通過。`handoffs` frontmatter 的 `agent:` 值應使用 `.agent.md` 的檔案名稱去掉副檔名（例如 `dba`、`frontend-designer`）。
...
**三個安裝入口均需同步排除 `gate-check`**（各自獨立驗收，任一遺漏都視為 AC 未通過）：

```powershell
# scripts/bootstrap.ps1 — $skillsToInstall 陣列排除
$excludedSkills = @('gate-check')

# tools/install-plan.ps1 — $skillsToInstall 陣列排除
$excludedSkills = @('gate-check')

# tools/install-apply.ps1 — Skill 複製邏輯排除
$excludedSkills = @('gate-check')
```
...
| 修改類型 | Token 增量上限 | 超出處理 |
|---------|-------------|---------|
| **格式調整**（加 Anti-Rationalization + Verification 兩個區塊）| ≤ 400 tokens | 精簡現有段落以抵消 |
| **新模式/Mode 加入**（如 agentic-eval Pre-Decision Mode）| ≤ 900 tokens | 新 Mode 細節移至 `references/`，body 只保留觸發條件和概要 |
| **全新 Skill 建立**（context-engineering 等）| ≤ 5000 tokens（L2 上限）| 超出須分拆或大量移至 references/ |

**驗證方式**：以 `tiktoken` 或 VS Code LLM Token Counter 實測，格式調整前後差值需符合對應級別上限。
...
| 機制 | 來源 | 落地位置 |
|------|------|--------|
| `verification-before-completion` 鐵律：沒有執行或機械檢查就不能聲稱完成 | superpowers | Story 1 AC（Verification 第一條必須是可執行或可機械檢查的步驟） |
...
2. **Agent Handoff 完整性**：9 個 Agent 的 Handoff 鏈路可完整追蹤（brainstorm → spec → plan → coder → code-reviewer 無斷層）；每個 Agent 的 Handoff 區塊同時存在 `handoffs` frontmatter（VS Code）和 body `## Handoff`（CLI）
3. **新 Skill 完整性**：3 個新建 Skill 各自包含 Process、Anti-Rationalization、Verification 三個必要區塊；6 個擴充現有 Skill 的新增區塊各包含觸發條件、步驟說明、Verification
4. **Skill 行為驗收**：3 個新建 Skill + 6 個擴充區塊各至少通過 1 個 adversarial pressure session（Pass/Fail 判準：靜態區塊存在 + AI 明確拒絕 + 提供合規替代路徑），驗收記錄存於 `skill-pressure-tests.md`
5. **bootstrap 乾淨性**：使用 **三個安裝入口**（bootstrap.ps1、install-plan.ps1、install-apply.ps1）部署的新專案中，`gate-check` 均不出現在 `.github/skills/` 目錄
6. **sync 一致性**：`agents/` + `skills/` + `AGENTS.md` 與 `.github/` 下的對應檔案 diff 為零（`gate-check` 的 source 保留在 `skills/gate-check/` 不計入 parity 範圍，此為設計預期）
7. **Token 預算合規**：格式調整型 Skill token 增量 ≤ 400 tokens；新模式加入型 ≤ 900 tokens（超出者已移至 `references/`）；以 `tiktoken` 實測為準
8. **spec-agent AC 品質**（Story 13）：specification SKILL.md 更新後，任意抽取 3 個 AC 範例驗證：（a）格式為 Observable Outcome（b）Specialist Lens Review 有 4 個視角輸出（c）詞彙鎖定步驟可正常觸發（d）references/ 中每個檔案在 SKILL.md body 均有明確引用語句
